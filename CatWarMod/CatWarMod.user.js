...
// @version      2.5
// @description  Полезные дополнения для catwar.su | Обновление 2.5!
// @author       Fredo14 & ScriptTeam
// @copyright    2019—2020, Хвойница & 2024 ScriptTeam (https://vk.com/cwscript)
// @downloadURL  https://openuserjs.org/install/CatWarScript/CatWar_Mod.user.js
// @updateURL    https://openuserjs.org/meta/CatWarScript/CatWar_Mod.meta.js
...
<svg
  ...
  const mainTableObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function () {
      ...
      skyObserver.observe($('#sky')[0], { attributes: true });
      mainTableObserver.disconnect();
    });
  });

  mainTableObserver.observe($('#main_table > tbody')[0], { childList: true });
...
<div id="cmSite">
    ...
    <div>
      ...
      <label for="blogAvatars">Аватарки в комментариях</label>
        <label for="blogAvatarsSize">Размер:</label>
        <input class="cwmod-settings" id="blogAvatarsSize" type="number" min="30" max="250" style="width: 45px" value="${globals.blogAvatarsSize}"> px
        <input class="cwmod-settings" id="blogAvatarsNoCrop" type="checkbox" ${globals.blogAvatarsNoCrop ? 'checked' : ''}>
        <label for="blogAvatarsNoCrop">Не обрезать до квадрата</label>
    ...
    <div>
      ...
      <label for="blogCiteButton">Кнопка «Цитировать комментарий»</label>
  ...
  <div id="cmGame">
    ...
    <div>
      ...
      <div>
        ...
        <label for="menuTop">СИ (список игроков)</label>
      </div>
      <div>
  ...
  <div id="cmStyle">
    ...
    <div>
      ...
      <label for="compact">Компактная игровая
        ...
        <label for="compactSwapSides">Поменять местами блоки (погода, действия, «во рту», чат справа)</label>
        <input class="cwmod-settings" id="compactChatOnTop" type="checkbox" ${globals.compactChatOnTop ? 'checked' : ''}>
        <label for="compactChatOnTop">Чат наверху</label>
        <input class="cwmod-settings" id="compactRadiusEdges" type="checkbox" ${globals.compactRadiusEdges ? 'checked' : ''}>
        <label for="compactRadiusEdges">Скруглить углы</label>
        <input class="cwmod-settings" id="compactSplitInfo" type="checkbox" ${globals.compactSplitInfo ? 'checked' : ''}>
        <label for="compactSplitInfo">Разделить параметры, историю и родственные связи
          <a href="#" id="about-split-info">(?)</a>
        </label>
    </div>
    <div>
      <label for="cw3Bkg">Фон страницы:</label>
      <select class="cwmod-settings" id="cw3Bkg">
        <option value="0">по умолчанию</option>
        <option value="1">фон локации</option>
        <option value="2">свой</option>
      </select>
        <label for="cw3BkgImg">Картинка:</label>
        <input class="cwmod-settings" id="cw3BkgImg" type="text" value="${globals.cw3BkgImg}">
        <label for="cw3BkgSize">Размер:</label>
        <select class="cwmod-settings" id="cw3BkgSize">
          <option value="0">автоматически</option>
          <option value="1">по размеру страницы</option>
        </select>
        <label for="cw3BkgPos">Положение:</label>
        <select class="cwmod-settings" id="cw3BkgPos">
          <option value="0">вверху слева</option>
          <option value="1">вверху по центру</option>
          <option value="2">вверху справа</option>
          <option value="3">по центру слева</option>
          <option value="4">по центру</option>
          <option value="5">по центру справа</option>
          <option value="6">внизу слева</option>
          <option value="7">внизу по центру</option>
          <option value="8">внизу справа</option>
        </select>
    </div>
    <div>
  ...
  <div id="cmLocation">
    ...
    <div>
      ...
      <label for="alwaysDay">Всегда день (убрать затемнение игрового поля)</label>
    </div>
  </div>
  <div id="cmChat">
    <h4>Чат</h4>
  ...
  <div id="cmImEx">
    ...
    <div class="cwmod-data-result" data-result="settings"></div>
      <input id="clear-ym-storage" type="button" value="Кнопка"> для тех, у кого ничего не сохраняется
